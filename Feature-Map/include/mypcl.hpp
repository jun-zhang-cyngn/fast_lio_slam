...
namespace mypcl
{
  ...
  void loadPCD(std::string filePath, int pcd_fill_num, pcl::PointCloud<cyngn::localization::PointXYZITR>::Ptr& pc, int num,
               std::string prefix = "")
  {
    std::stringstream ss;
    if(pcd_fill_num > 0)
  ...
  void transform_pointcloud(pcl::PointCloud<cyngn::localization::PointXYZITR> const& pc_in,
                            pcl::PointCloud<cyngn::localization::PointXYZITR>& pt_out,
                            Eigen::Vector3d t,
                            Eigen::Quaterniond q)
  {
